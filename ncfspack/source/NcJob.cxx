...
//        and the main processing object are NOT deleted by this base class.
 ...
 if (fFolder)
 {
  ...
  if (list)
  {
   TFolder* top=(TFolder*)list->FindObject("NcJob-folders");
...
// Store pointer of specified object in the working environment.
 ...
 {
  // Create the top level environment folder for all NcJobs if needed 
  TList* list=gROOT->GetListOfBrowsables();
  if (list)
  {
   TFolder* top=(TFolder*)list->FindObject("NcJob-folders");
   if (!top)
   {
    top=new TFolder("NcJob-folders","Environment for all NcJob derived tasks");
    list->Add(top,"NcJob-folders");
   }
