...
{
 public:
  NcAttrib();                                  // Default constructor
  virtual ~NcAttrib();                         // Destructor
  NcAttrib(const NcAttrib& a);                 // Copy constructor
  ...
  virtual void Load(NcAttrib& a,Int_t j=0);   // Load j-th slot or all attributes of the input NcAttrib
  virtual void Load(NcAttrib& a,TString name);// Load name-specified slot attributes of the input NcAttrib
  ...
  Int_t GetSlotIndex(TString name,Int_t opt=0) const; // Provide the slot index of the matching name
 ...
 ClassDef(NcAttrib,2) // Generic handling of detector signal (calibration) attributes.
};
#endif
