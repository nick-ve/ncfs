...
class NcSignal : public TNamed,public NcPosition,public NcAttrib
{
 public:
  ...
  void RemoveTracks(Int_t mode=1);                              // Remove all related NcTracks from this signal
  Int_t GetNtracks(NcTrack* t=0) const;                         // Provide number of related NcTracks
  NcTrack* GetTrack(Int_t j) const;                             // Access to the related NcTrack number j
  NcTrack* GetIdTrack(Int_t id) const;                          // Access to the related NcTrack with ID=id
  ...
  Bool_t fDevset;                // Flag to allow the setting of a device as owner (kTRUE) or not (kFALSE)
