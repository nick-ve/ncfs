...
// Destructor to delete dynamically allocated memory
 if (fLinks)
 {
  ...
  TArrayI js;
  TArrayI ks;
  Int_t nlinks=GetIndices(0,js,ks);
  Int_t j=0;
  Int_t k=0;
  for (Int_t il=0; il<nlinks; il++)
  {
   j=js.At(il);
   k=ks.At(il);
   TObject* obj=GetLink(j,k);
   if (!obj) continue;
   if (obj->InheritsFrom("NcDevice"))
   {
    NcDevice* dx=(NcDevice*)obj;
    dx->RemoveHit(this);
   }
  }
  delete fLinks;
...
//
 ...
 if (mode<0 || mode>1)
 {
  cout << " *NcSignal::Reset* Invalid argument mode = " << mode << endl;
...
// The default when invoking ResetSignals() corresponds to mode=0.
 ...
 if (abs(mode)>2)
 {
  cout << " *NcSignal::ResetSignals* Invalid argument mode = " << mode << endl;
  cout << " Default mode=0 will be used." << endl;
  mode=0;
 }
...
// The default when invoking DeleteSignals() corresponds to mode=0.
 ...
 if (abs(mode)>2)
 {
  cout << " *NcSignal::DeleteSignals* Invalid argument mode = " << mode << endl;
  cout << " Default mode=0 will be used." << endl;
  mode=0;
 }
...
// The default is mode=0.
 ...
 Int_t pj=0;
 Int_t pdead=0;
 NcSignal* parent=(NcSignal*)GetDevice();
 ...
 {
  sx=(NcSignal*)GetDevice();
  if (pj) jcal=pj;
 }
 if (!sx && abs(mode)>=5) sx=(NcSignal*)this;
 if (mode==5) mode=2;
 if (mode==-5) mode=-2;
 if (mode==6) mode=3;
 if (mode==-6) mode=-3;
 ...
 if (fSignals)
 {
  ...
  {
   cout << " *NcSignal::GetSignal* Index j = " << j << " invalid." << endl;
  } 
 }
 return sig;
}
...
// either directly or via a few invokations of GetSlotIndex().
 ...
 Int_t j=GetSlotIndex(name);
 Float_t val=0;
 if (j>0 && j<=(fSignals->GetSize())) val=GetSignal(j,mode);
 return val;
}
///////////////////////////////////////////////////////////////////////////
...
// In case no signal is present or the argument j is invalid, 0 is returned.
 ...
 if (fDsignals)
 {
  ...
  {
   cout << " *NcSignal::GetSignalError* Index j = " << j << " invalid." << endl;
  } 
 }
 return err;
}
...
// either directly or via a few invokations of GetSlotIndex().
 ...
 Int_t j=GetSlotIndex(name);
 Float_t val=0;
 if (j>0 && j<=(fDsignals->GetSize())) val=GetSignalError(j);
 return val;
}
///////////////////////////////////////////////////////////////////////////
...
// information will be suppressed.
 ...
 if (j<-1) 
 {
  cout << " *NcSignal::List* Invalid argument j = " << j << endl;
  return;
 }
...
// information will be suppressed.
 ...
 if (j<-1) 
 {
  cout << " *NcSignal::ListWaveform* Invalid argument j = " << j << endl;
  return;
 }
...
// information will be suppressed.
 ...
 if (j<-1) 
 {
  cout << " *NcSignal::ListTrack* Invalid argument j = " << j << endl;
  return;
 }
...
// The value stored is : 10*signalflag + errorflag.
 ...
 if (j<1) 
 {
  cout << " *NcSignal::SetSigFlags* Invalid argument j = " << j << endl;
  return;
 }
...
// In case j is invalid, 0 is returned.
 ...
 if (j<1) 
 {
  cout << " *NcSignal::GetSignalFlag* Invalid argument j = " << j << endl;
  return 0;
 }
 Int_t flag=0;
 if (fSigflags)
...
// In case j is invalid, 0 is returned.
 ...
 if (j<1) 
 {
  cout << " *NcSignal::GetErrorFlag* Invalid argument j = " << j << endl;
  return 0;
 }
 Int_t flag=0;
 if (fSigflags)
...
// In case no match is found, zero is returned.
 ...
 for (Int_t i=1; i<=n; i++)
 {
  ...
  if (waveform)
  {
   ...
   if (str.Contains(name)) return waveform;
  }
 }
...
//         j=0 ==> All waveforms will be reset.
 ...
 {
  cout << " *NcSignal::ResetWaveform* Index j = " << j << " invalid." << endl;
  return;
 }
}
///////////////////////////////////////////////////////////////////////////
...
//         j=0 ==> All waveforms will be deleted.
 ...
 {
  cout << " *NcSignal::DeleteWaveform* Index j = " << j << " invalid." << endl;
  return;
 }
}
///////////////////////////////////////////////////////////////////////////
...
// to prevent the situation of stored pointers to non-existent object. 
 ...
 if (obj) 
 {
  if (obj->InheritsFrom("NcTrack"))
  {
   NcTrack* t=(NcTrack*)obj;
   AddTrack(*t,1);
  }
 }
}
...
// This facility may only be used internally by NcDevice (or derived) objects.
 ...
 {
  cout << " *" << ClassName() << "::SetDevice* Invokation by public users is not allowed." << endl;
 }
}
///////////////////////////////////////////////////////////////////////////
...
// Note : j=1 denotes the first track.
 ...
 if ((j >= 1) && (j <= GetNtracks()))
 {
  return (NcTrack*)fTracks->At(j-1);
 }
 else
 {
  cout << " *NcSignal* track number : " << j << " out of range."
       << " Ntk = " << GetNtracks() << endl;
  return 0;
 }
}
