...
class RnoGANT : public NcDevice
{
 public:
  ...
  Int_t GetNumber(Int_t id=0) const;                 // Provide the corresponding antenna or DAQ channel number
  Int_t GetANTId(Int_t station,Int_t string,Int_t number) const; // Provide unique ID based on station, string and number indicators
 ...
 ClassDef(RnoGANT,3) // Signal (Hit) handling of an RNO-G Generic Antenna (GANT).
};
#endif
